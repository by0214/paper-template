...
\subsection{Proof Sketches for Local Credit Lemmas}
...
\(
\phi_{i,t}^{\mathrm{ord}}=v_i(P_{i,t})-v_i(P_{i,t-1})
\).
\end{proof}

\begin{proof}[Proof sketch of \Cref{prop:reward_shaping}]
Let
\(
...
\),
so these dense rewards only redistribute the same trajectory-level signal
across steps.
Now let \(r_{i,t}^{\mathrm{sparse}}\) denote the original sparse reward, which
is zero on intermediate steps and non-zero only at the terminal outcome.
If \(v_i(P_{i,t})\) is identified with a potential function
\(\Phi(P_{i,t})\), then the shaped reward can be written as
\(
r_{i,t}^{\mathrm{shape}}
=
r_{i,t}^{\mathrm{sparse}}
+
\Phi(P_{i,t})-\Phi(P_{i,t-1})
\).
By the policy-invariance theorem of \citet{ng1999policy}, such a
potential-based transformation preserves the set of optimal policies.
Therefore, ordered value differences alone do not inject additional
optimal-policy information beyond the original sparse reward.
\end{proof}
